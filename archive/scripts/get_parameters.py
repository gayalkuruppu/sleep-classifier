...
from source.algorithms.MyGradientBoostingClassifier import MyGradientBoostingClassifier
...
def find_best(method_key, feature_set, training_subjects):
    ...
    # if method_key == 'SVM':
    #     parameters = {'kernel': ['rbf'], 'gamma': [0.001, 0.01, 0.1, 1, 10, 100], 'C': [0.1, 1, 10, 100]}
    #     classifier = SVC()

    if method_key == 'Naive Bayes':
        parameters = {'alpha': [0.1, 0.01, 0.001, 0.0001, 0.00001]}
        classifier = MultinomialNB()

    if method_key == 'My Gradient Boosting':
        parameters = {'max_depth': [10, 50, 100], 'n_estimators': [100, 500, 1000], 'learning_rate': [0.01, 0.1, 1]}
        classifier = MyGradientBoostingClassifier(max_depth=10, n_estimators=100, learning_rate=0.01, response_method='predict_proba')
